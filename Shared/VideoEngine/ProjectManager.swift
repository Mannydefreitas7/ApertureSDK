...
struct FilterData: Codable {
    ...
    init(from filter: VideoFilter) {
        self.id = filter.id
        self.type = filter.type.rawValue
        self.intensity = filter.intensity
        self.brightness = filter.parameters.brightness
        self.contrast = filter.parameters.contrast
        self.saturation = filter.parameters.saturation
        self.exposure = filter.parameters.exposure
        self.temperature = filter.parameters.temperature
        self.tint = filter.parameters.tint
        self.highlights = filter.parameters.highlights
        self.shadows = filter.parameters.shadows
        self.vibrance = filter.parameters.vibrance
        self.sharpness = filter.parameters.sharpness
        self.vignette = filter.parameters.vignette
        self.grain = filter.parameters.grain
    }

    func toVideoFilter() -> VideoFilter {
        ...
        return VideoFilter(
            id: id,
            type: FilterType(rawValue: type) ?? .none,
            intensity: intensity,
            parameters: params
        )
    }
...
struct TextOverlayData: Codable {
    ...
    init(from overlay: TextOverlay) {
        ...
        self.fontSize = overlay.style.fontSize
        self.fontWeight = overlay.style.fontWeight.rawValue
        self.textColorRed = overlay.style.textColor.red
        self.textColorGreen = overlay.style.textColor.green
        self.textColorBlue = overlay.style.textColor.blue
        self.textColorAlpha = overlay.style.textColor.alpha
    ...
    func load(from url: URL) async throws -> Project {
        ...
        for trackData in projectData.tracks {
            ...
            for clipData in trackData.clips {
                ...
                clip.filter = clipData.filter?.toVideoFilter()
        ...
        if includeMedia {
            ...
            for track in project.tracks {
                for clip in track.clips {
                    let destURL = mediaDir.appendingPathComponent(clip.sourceURL.lastPathComponent)
                    if !FileManager.default.fileExists(atPath: destURL.path) {
                        try FileManager.default.copyItem(at: clip.sourceURL, to: destURL)
                    }
                }
            }
        }
